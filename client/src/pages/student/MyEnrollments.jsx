    ...
    <div className='md:px-36 px-8 pt-10 flex-1'>
      <div className='flex items-center justify-between mb-6'>
        ...
        <button
          ...
          className='inline-flex items-center gap-2 px-4 py-2 bg-linear-to-r from-orange-500 to-amber-500 text-white text-sm rounded-lg hover:from-orange-600 hover:to-amber-600 transition-all shadow-md'
        >
      ...
      <table className='md:table-auto table-fixed w-full overflow-hidden border mt-4'>
        <thead className='text-gray-900 border-b border-gray-500/20 text-sm text-left max-sm:hidden'>
          <tr>
            ...
            <th className='px-4 py-3 font-semibold truncate'>Thời lượng</th>
            <th className='px-4 py-3 font-semibold truncate'>Tiến độ</th>
        ...
        <tbody className='text-gray-700'>
          { enrolledCourses.map((course, index) => (
            <tr key={index} className='border-b border-gray-500/20'>
              ...
              <td className='px-4 py-3 max-sm:hidden'>
                {progressArray[index] && `${progressArray[index].lectureCompleted} / ${progressArray[index].totalLectures}`} <span>bài giảng</span>
              </td>
              <td className='px-4 py-3 max-sm:text-right'>
                <button onClick={() => navigate('/player/' + course._id)} className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:text-xs text-white'>
                  {progressArray[index] && progressArray[index].lectureCompleted / progressArray[index].totalLectures === 1 ? 'Hoàn thành' : 'Tiếp tục học'}
                </button>
              </td>
            </tr>
          ))}
